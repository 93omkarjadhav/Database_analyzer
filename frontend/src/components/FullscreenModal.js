...
function FullscreenModal({ fullscreenData, setFullscreenData }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-10 backdrop-blur-sm">
      <div className="max-h-full w-full overflow-auto rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-6 shadow-2xl">
        <div className="mb-6 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          ...
          <button
            ...
            className="rounded-lg bg-slate-100 dark:bg-slate-800 px-4 py-2 text-xs font-bold text-rose-400 transition hover:bg-rose-500 hover:text-white"
          >
            CLOSE [ESC]
          </button>
        </div>
        <table className="min-w-full border-collapse text-left text-sm text-slate-800 dark:text-slate-100">
          ...
          <tbody>
            {fullscreenData.map((row, i) => (
              <tr
                key={i}
                className="border-b border-slate-200/50 dark:border-slate-800/50 hover:bg-white/5"
              >
                {Object.values(row).map((v, j) => (
                  <td
                    key={j}
                    className="border-r border-slate-200/30 dark:border-slate-800/30 px-4 py-3 font-mono text-xs"
                  >
                    {String(v)}
                  </td>
                ))}
